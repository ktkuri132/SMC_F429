...
/// @brief
/// 将从串口读出的数据保存到CamerData中,原本读取数字是不用获取坐标信息的,但是当时写错了,后来也就没改过来,函数的运行条件是SaveDataLock必须为0
...
/// @return <0 未完成
/// @return >0 完成
int8_t __attribute__((__weak__)) Data_Save_from_Camer()
{
    static uint8_t i = 0;
    if (!SaveDataLock)
    {
...
/// @brief
/// 将从串口识别到的数字进行识别配对,函数运行的第一优先级是SaveDataLock必须为1,第二优先级是VerifyDataLock必须为0
/// @return >0 得到方向 <0 未完成
int8_t __attribute__((__weak__)) Data_Get_from_Camer()
{
    if (SaveDataLock)
    {
        static uint8_t m = 0;
